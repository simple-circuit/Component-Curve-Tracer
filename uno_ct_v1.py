...
def plotxy(xtra = 0):
    ...
    if xtvar.get() == False:
        ...
        canvas.create_oval(253+x[n][a]*22.07, 253-y[n][a]*51.2, 259+x[n][a]*22.07, 259-y[n][a]*51.2, fill='', outline='white', width=1)               
    else:
        for j in range(n2,m):
            for i in range(255):       
        ...
        canvas.create_oval(a*2-3, 253-x[n][a]*22.07, a*2 + 3, 259-x[n][a]*22.07, fill='', outline='white', width=1)               
        canvas.create_oval(a*2-3, 253-y[n][a]*51.2, a*2 + 3, 259-y[n][a]*51.2, fill='', outline='white', width=1)               
...
cursor = Spinbox(canvas, from_= 0, to = 255, width = 4, command = plotxy, textvariable = curvar)
cursor.place(x = 520, y = 255)
cursor.bind("<Return>", plotxy)
